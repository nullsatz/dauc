...
__global__ void d_auctionAssign(const int nBidders, const int nItems,
...
__global__ void d_auctionBid(const int nBidders, const int nItems,
...
int * d_auction(int nBidders, int nItems, float * h_benefits) {
	...
	*h_anyUnassigned = 1;
	...
	while(*h_anyUnassigned > 0) {
		...
		d_auctionBid<<<dimBlock, dimGrid>>>(nBidders, nItems, d_bids,
		...
		if(*h_anyUnassigned > 0) {
			d_auctionAssign<<<dimBlock, dimGrid>>>(nBidders, nItems,
	...
	// Read back GPU results: read the assignements from d_bidderItems
	cudaMemcpy(h_bidderItems, d_bidderItems, biddersISize,
		cudaMemcpyDeviceToHost);
 
	cudaFree(d_bidderItems);

	// stop timing for performance profiling
